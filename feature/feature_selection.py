...
class PMISelection(FeatureSelection):
    def __init__(self, feature_mat, targets, pmi_ratio=0.06):
        FeatureSelection.__init__(self, feature_mat, targets)
        self.pmi_ratio = pmi_ratio
    ...
    def get_boolean_selection_lst(self):
        ...
        if not self.feature_filtered:
            ...
            x = []
            s = []
            print("PMISelection: compute multual begin!")
            for i in range(len(features[0])):
                for j in range(len(features)):
                    x.append(features[j][i])
                s.append(mr.mutual_info_score(label, x))
                x[:] = []
            print("PMISelection: compute multual end!")

            for i in range(len(s)):
                if s[i] < self.pmi_ratio:
...
class GBDTSelection(FeatureSelection):
    def get_boolean_selection_lst(self):
        if not self.feature_filtered:
            ...
            self.feature_mat = [True if item > 0 else False for item in gbdt.feature_importances_]
        return self.feature_filtered
